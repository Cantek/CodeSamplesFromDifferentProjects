...
#define CLIENT_NAME		_T("Akpacs CD Burner")

// AkPacsCDBurnerCDDlg dialog

CBurnCDDlg::CBurnCDDlg(CWnd* pParent)
...
CBurnCDDlg::CBurnCDDlg(CWnd* pParent,std::wstring cmdParams)
: CDialog(CBurnCDDlg::IDD, pParent)
, m_closeMedia(TRUE)
, m_cancelBurn(false)
, m_selectedMediaType(-1)
...
void CBurnCDDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CAPACITY2, m_capacityProgress2);
	DDX_Control(pDX, IDC_MAX_TEXT, m_maxText);
	DDX_Control(pDX, IDC_MAX_TEXT2, m_maxText2);
...
void CBurnCDDlg::prepareForAdd()
{
	size_t pos_start = 0;
	size_t pos_end = 0;
	while( (pos_end = m_path.find('#',pos_start) ) != std::wstring::npos )
	{
		//pos_end = m_path.find('#',pos_start);
		//MessageBoxW(m_path.substr(pos_start, pos_end ).c_str(),NULL,0);
		CString newPath = m_path.substr(pos_start, pos_end - pos_start+1 ).c_str();
		//MessageBoxW(newPath,NULL,0);
		newPath.Replace('#','/');
		//newPath.Replace("\"","");
		//newPath.Replace("","");
		addFoldersToList(newPath);
		pos_start = pos_end + 1;
	}		
	//MessageBoxW(m_path.substr(pos_start, m_path.length() - 2 ).c_str(),NULL,0);
	//if(m_path.length() > pos_start)
	//	addFoldersToList(m_path.substr(pos_start, m_path.length() - 2 ).c_str());	
}
...
void CBurnCDDlg::addFoldersToList(CString path)
{
	//MessageBoxW(path,NULL,0);
	int selectedIndex = this->m_deviceComboBox.GetCurSel();
	ASSERT(selectedIndex >= 0);
	if (selectedIndex < 0)
	{
		MessageBox(L"You do not have any installed media recorder device.\n You can not use AkPacs Cd Burner without an installed Media Recorder!",L"No Installed Recorder",0);
		this->CloseWindow();
	}
	else
	{
		CDiscRecorder* pOrigDiscRecorder = 
			(CDiscRecorder*)this->m_deviceComboBox.GetItemDataPtr(selectedIndex);
		if (pOrigDiscRecorder == NULL)
		{
			//this->SendMessage(WM_BURN_FINISHED, 0, (LPARAM)_T("Error: No Data for selected device.\n Be sure that device is working!"));
			MessageBox(L"You do not have any installed media recorder device.\n You can not use AkPacs Cd Burner without an installed Media Recorder!",L"No Installed Recorder",0);
			this->CloseWindow();
			//return;
		}
			...
			if (discRecorder.Initialize(pOrigDiscRecorder->GetUniqueId()))
			{
				if (discRecorder.AcquireExclusiveAccess(true, CLIENT_NAME))
				{
					...
					if (discFormatData.Initialize(&discRecorder, CLIENT_NAME))
					{
						...
						long spaceFree = discFormatData.getFreeSpaceOfMedia();
						long spaceTotal = discFormatData.getTotalSpaceOfMedia();
						m_mbSpaceFree = ((((double)spaceFree*2048)/1000)/1000);
						m_mbSpaceTotal = ((((double)spaceTotal*2048)/1000)/1000);
						if(m_mbSpaceTotal > 4000 && m_mbSpaceTotal < 8000)
						{
							m_mediaTypeCombo.SetCurSel(1);
						}
						if(m_mbSpaceTotal > 600 && m_mbSpaceTotal < 800)
						{
							m_mediaTypeCombo.SetCurSel(0);
						}
						if(m_mbSpaceTotal > 7000 && m_mbSpaceTotal < 10000)
						{
							m_mediaTypeCombo.SetCurSel(2);
						}
						if(m_mbSpaceFree == m_mbSpaceTotal &&  m_mbSpaceTotal != 0)
						{
							path.Replace('/','\\');
							LPITEMIDLIST itemidlist;
							GetItemIdListFromPath(path.GetBuffer(0),&itemidlist);
							TCHAR selectedPath[_MAX_PATH] = {0};
							if(SHGetPathFromIDList(itemidlist, selectedPath))
							{
								CDirObject* pDirObject = new CDirObject(path);		
								int addIndex = m_fileListbox.AddString(pDirObject->GetName());
								m_fileListbox.SetItemDataPtr(addIndex, pDirObject);
								OnCbnSelchangeMediaTypeCombo();
								EnableBurnButton();
						...
						{
							MessageBox(L"Media has not enough space or there is no media in recoder device for writing selected files!",L"Not enough Space - No Device",0);
							DestroyWindow();
						}
					}
					else
					{
						MessageBox(L"Error on initialization of installed Media Recorder device\nYou can not use AkPacs Cd Burner without an accessable installed Media Recorder!",L"Error Installed Recorder",0);
						DestroyWindow();
					}
				}
				else
				{
					MessageBox(L"Your installed media recorder device is not accessable\nYou can not use AkPacs Cd Burner without an accessable installed Media Recorder!",L"Error Installed Recorder",0);
					DestroyWindow();
				}
			}
			else
			{
				MessageBox(L"Your installed media recorder device could not initialized\nYou can not use AkPacs Cd Burner without a working installed Media Recorder!",L"Error Installed Recorder",0);
				DestroyWindow();
			}
		}
	}
...
BOOL CBurnCDDlg::GetItemIdListFromPath (LPWSTR lpszPath, LPITEMIDLIST *lpItemIdList)
{
...
ITEMIDLIST* CBurnCDDlg::PathToPidl(CString path)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		for (int itemIndex = 0; itemIndex < itemCount; itemIndex++)
		{
			...
			else if (pObject->IsKindOf(RUNTIME_CLASS(CDirObject)))
			{
				...
				CString ali = pDirObject->GetPath();
				BSTR veli  = pDirObject->GetPath().AllocSysString();
...
void CBurnCDDlg::UpdateCapacity()
{
	...
	std::string totalSpaceOfMedia;
	{ 
		std::ostringstream ss;
		ss << (int)m_mbSpaceTotal;
		totalSpaceOfMedia = ss.str();
	}
	CString totalSpaceOfMediaC(totalSpaceOfMedia.c_str());
	totalSpaceOfMediaC.Append(L" MB");
	m_maxText2.SetWindowText(totalSpaceOfMediaC);
	m_maxText.SetWindowText(totalSpaceOfMediaC);

	std::string loadedSpaceOfMedia;
	{ 
		std::ostringstream ss;
		ss << (int)(m_mbSpaceTotal - m_mbSpaceFree);
		loadedSpaceOfMedia = ss.str();
	}

	CString loadedSpaceOfMediaC(loadedSpaceOfMedia.c_str());
	loadedSpaceOfMediaC.Append(L" MB");
	m_staticText2.SetWindowText(loadedSpaceOfMediaC);
	...
	for (int itemIndex = 0; itemIndex < itemCount; itemIndex++)
	{
		...
		mediaSize += ((pObject->GetSizeOnDisc())/1000)/1000;
	}
	int lastLoadedSize = (int)(mediaSize + m_mbSpaceTotal - m_mbSpaceFree);
	std::string loadedSpaceOnMediaAfterAdd;
	{ 
		std::ostringstream ss;
		ss << lastLoadedSize;
		loadedSpaceOnMediaAfterAdd = ss.str();
	}
	CString loadedSpaceOnMediaAfterAddC(loadedSpaceOnMediaAfterAdd.c_str());
	loadedSpaceOnMediaAfterAddC.Append(L" MB");
	m_staticText.SetWindowText(loadedSpaceOnMediaAfterAddC);
	
	m_capacityProgress.SetRange(0,100);
	m_capacityProgress2.SetRange(0,100);

	mediaSize += m_mbSpaceTotal - m_mbSpaceFree;
	if (mediaSize == 0)
	{
	...
	{
		int percent = (int)((mediaSize*100)/m_mbSpaceTotal);
		int percent2 = (int)(((m_mbSpaceTotal-m_mbSpaceFree)*100)/m_mbSpaceTotal);
		if (percent2 > 100)
		{
			m_capacityProgress2.SetPos(100);
			#if _MFC_VER >= 0x0900
		...
		{
			m_capacityProgress2.SetPos(percent2);
			#if _MFC_VER >= 0x0900
			//m_capacityProgress.SetState(PBST_NORMAL);
			#endif
		}
