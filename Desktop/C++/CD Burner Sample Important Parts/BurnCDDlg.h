...
public:
	...
	CBurnCDDlg(CWnd* pParent /*=NULL*/,std::wstring ali);
// Dialog Data
	enum { IDD = IDD_BURNCD_DIALOG };
	std::wstring m_path;

	double m_mbSpaceTotal;
	double m_mbSpaceFree;

	void addFoldersToList(CString path);
	ITEMIDLIST* PathToPidl(CString path);
...
protected:
	...
	CProgressCtrl m_capacityProgress2;
	CStatic m_maxText;
	CStatic m_maxText2;
